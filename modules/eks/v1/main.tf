...
provider "helm" {
  kubernetes {
    ...
    exec {
      ...
      command     = "aws"
      args        = ["eks", "get-token", "--cluster-name", var.project-name, "--region", var.region]
    }
  }
}
...
resource "kubectl_manifest" "karpenter_provisioner_template" {
  yaml_body  = <<-YAML
...
resource "aws_iam_policy" "node-not-ready-policy" {
  name        = "${var.project-name}-Karpenter-Node-NotReady-policy"
  description = "Used in Karpenter"
  path        = "/"
  policy      = <<EOT
{
    "Version": "2012-10-17",
